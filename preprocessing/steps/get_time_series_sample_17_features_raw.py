...
# ## Get time series for 17 raw features
#
# This script is used for generating time series for 17 raw features.
...
def get_time_series_sample_17_features_raw_Xhrs(args, hrs):
    ...
    map_feature_colids = np.load(cachedir.joinpath('admdata_17f/raw', 'map_feature_colids.npy'), allow_pickle=True).tolist()
    map_feature_colids
    ...
    # urine: sum: 1*1
    # static features! 5*1
    min_list = list(map(lambda x: map_feature_colids[x], [
                    'gcsverbal', 'gcsmotor', 'gcseyes']))
    minmax_list = list(map(lambda x: map_feature_colids[x], ['systolic_blood_pressure_abp_mean', 'heart_rate', 'body_temperature', 'serum_urea_nitrogen_level',
                                                             'white_blood_cells_count_mean', 'serum_bicarbonate_level_mean', 'sodium_level_mean', 'potassium_level_mean', 'bilirubin_level']))
    minmaxavg_list = list(
    ...
    for i in range(N):
        ...
        hrs_mean_array[i, fstart:fstart +
                       len(minmax_list)*2] = np.concatenate([tempmin, tempmax])
...
def get_time_series_sample_17_features_raw(args):
    get_time_series_sample_17_features_raw_Xhrs(args, 24)
    get_time_series_sample_17_features_raw_Xhrs(args, 48)
